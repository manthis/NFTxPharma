...
const { getPatientsHexMerkleRoot, getPatientsTreeProof } = require('./merkletrees');
...
describe('Patient', function () {
    ...
    describe('Deployment', function () {
        ...
        it('should have its merkleroot initialized', async function () {
            ...
            expect(await patient.merkleRoot()).to.equal(getPatientsHexMerkleRoot());
        });
    });
    ...
    describe('Setters', function () {
        it('should set merkle root as admin', async function () {
            ...
            expect(await patient.merkleRoot()).to.equal(newRoot);
        });
    ...
    describe('Authorization', function () {
        it('should authorize a valid patient', async function () {
            const { patient, addrs } = await loadFixture(deployContractFixture);
            expect(await patient.isWhitelisted(addrs[10].address, getPatientsTreeProof(addrs[10].address))).to.equal(
                true,
            );
        });
        ...
        it('should deny an invalid patient', async function () {
            ...
            expect(await patient.isWhitelisted(addr.address, getPatientsTreeProof(addr.address))).to.equal(false);
        });
    });
});
